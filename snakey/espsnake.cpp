...
void initSerial(const char* portName) {
    hSerial = CreateFile(portName, GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
int main(){
    ...
    if (hSerial == INVALID_HANDLE_VALUE){
        ...
        std::cout << "Connected to ESP32. Waiting for boot..." << std::endl;
        Sleep(2000); //Give the ESP32 2 seconds to reboot and start its loop
    }
